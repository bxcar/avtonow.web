...
?>
    ...
    <main class="main" id="main">
        <div class="main__container main__container--catalog">
            ...
            <div class="catalog-content">
                <p class="catalog-content__breadcrumbs">Главная / <?= single_cat_title(); ?></p>
                <h1 class="catalog-content__title"><?= single_cat_title(); ?></h1>
                <?php
                if ($_GET['showall'] == 1) {
                    $posts_per_page = -1;
                } else {
    ...
    <script>
        ...
        $child = get_category($category->term_id);

        //from your child category, grab parent ID
        $parent = $child->parent;
        if($parent) { ?>
        jQuery('.menu-category-<?= $parent ?>').toggleClass('active');
        jQuery('.menu-category-<?= $parent ?>').find('.sub-menu').animate({height: 'show'}, 300);
        <?php } else { ?>
        jQuery('.menu-category-<?= $category->term_id ?>').toggleClass('active');
        jQuery('.menu-category-<?= $category->term_id ?>').find('.sub-menu').animate({height: 'show'}, 300);
        <?php } ?>
        location.hash = 'main';
    </script>
